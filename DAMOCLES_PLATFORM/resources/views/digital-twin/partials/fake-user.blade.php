        ...
        @foreach ($users as $user)
            <div class="user-card border border-sky-700 rounded-lg p-4 bg-white shadow-sm w-64 transition-all cursor-pointer"
                ...
                <div class="flex items-center justify-between">
                    <h3 class="font-semibold user-name">{{ $user->name }} {{ $user->surname }}</h3>
                    <input id="user{{ $user->id }}" type="checkbox" class="user-checkbox hidden"
                        data-user-id="{{ $user->id }}">
                </div>
                <p class="text-sm">@lang('training-campaign.users.dob'):
                    {{ \Carbon\Carbon::parse($user->dob)->format('d/m/Y') }}</p>
                <p class="text-sm">@lang('training-campaign.users.gender'): {{ $user->gender }}</p>
                <p class="text-sm">@lang('training-campaign.users.companyRole'): {{ $user->company_role }}</p>
            </div>
        @endforeach
    </div>
</section>
