<x-app-layout>
    ...
    @php
        $submitRoute = route('email-groups.update-group');
    @endphp
    <div class="mt-[-3rem]">
        @include('layouts.partials.emails-selection', [
            'Title' => __('general.selectEmails'),
            'Finish' => __('general.endEditing'),
...
<script>
    // Pass the selected email IDs of the group to be self-checked
    const selectedEmails = @json($emails->where('isSelected', true)->pluck('id'));
    console.log("Selected Emails:", selectedEmails);

    document.addEventListener('DOMContentLoaded', function() {

        document.querySelectorAll('.email-checkbox').forEach(function(checkbox) {
            const emailId = checkbox.getAttribute('data-email-id');

            if (selectedEmails.includes(parseInt(emailId))) {
