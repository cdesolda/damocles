        ...
        <div class="pt-4">
            <section>
                ...
                <form id="updateForm" method="post" action="{{ route('profile.updateFromAdmin') }}" class="space-y-2">
                    ...
                    <div class="mt-4">
                        <x-input-label for="type" :value="__('user.partials.update.value.type')" />
                        <select id="type" name="type"
                            class="mt-1 block border-sky-800 focus:border-sky-800 focus:ring-sky-800 rounded-md shadow-sm w-full"
                            required autofocus autocomplete="type">
                            <option value="Real" {{ $user->type === 'Real' ? 'selected' : '' }}>
                                @lang('user.partials.update.real')
                            </option>
                            <option value="Fake" {{ $user->type === 'Fake' ? 'selected' : '' }}>
                                @lang('user.partials.update.fake')
                            </option>
                            <option value="DigitalTwin" {{ $user->type === 'DigitalTwin' ? 'selected' : '' }}>
                                @lang('user.partials.update.digitalTwin')
                            </option>
                        </select>
                    ...
                    <div class="flex justify-end items-center pt-4 gap-4">
                        @if (session('status') === 'profile-updated')
                            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
                                class="text-sm text-sky-600">@lang('user.partials.update.saved')</p>
                        @endif
