            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-8 text-sky-900">
                ...
                <div class="flex flex-col md:flex-row space-y-2 md:space-y-0 sm:justify-between items-center mb-6">
                    <p class="font-semibold text-xl w-full">
                        @lang('user.users') @if ($role && $role !== 'user')
                            - {{ ucfirst($role) }}
                        @endif
                    </p>
                            ...
                            @if ($users->count() > 0)
                                ...
                                <div class="flex flex-row w-80 gap-3 justify-end items-center relative">
                                    ...
                                    <input type="text" id="filter" name="filter"
                                        ...
                                        placeholder="@lang('general.enterSearch')">
                ...
                <!-- Content -->
                @if ($users->count() === 0)
                    <p class="text-center">@lang('general.noResult')</p>
                    ...
                    <table id="user-table" class="min-w-full text-center border-collapse">
                        <thead class="bg-gray-100">
                            <tr class="border-b-2 border-gray-300">
                                ...
                                <th class="py-2 px-4">@lang('general.user.role')</th>
                                <th class="py-2 px-4">@lang('general.user.companyRole')</th>
                                <th class="py-2 px-4">@lang('general.user.accept')</th>
                                <th class="py-2 px-4">@lang('general.user.state')</th>
                                            ...
                                            <x-slot name="content">
                                                <div class="flex flex-col gap-2">
                                                    <button class="hover:bg-gray-100 inner-element py-1"
                                                        ...
                                                        @click="$dispatch('open-modal', 'accept-modal');
                                                            fillAcceptModal({
                                                                id: '{{ $user->id }}',
                                                                value: '{{ $user->name }}',
                                                            });
                                                    ...
                                                    @if ($user->is_accept == true)
                                                        <button class="hover:bg-gray-100 inner-element py-1"
                                                            data-id="{{ $user->id }}" x-data=""
                                                            @click="$dispatch('open-modal', 'active-modal', { id: {{ $user->id }} })">
                                                            @if ($user->is_active)
                                                                @lang('user.disable')
                                                            @else
                                                                @lang('user.active')
                                                    ...
                                                    @if ($user->role !== 'Admin')
                                                        <button
                                                            ...
                                                            data-id="{{ $user->id }}" x-data=""
                                                            @click="$dispatch('open-modal', 'delete-modal', { id: {{ $user->id }} })">@lang('general.delete')</button>
                                                    @endif
                                                </div>
                                            </x-slot>
                                        </x-dropdown>
...
<!-- Accept modal -->
<x-modal name="accept-modal" id="accept-modal" title="Accept User" :show="false">
    <div class="p-4 rounded-lg relative">
        @include('user.partials.accept-user')
    </div>
</x-modal>

<!-- Active modal -->
<x-modal name="active-modal" id="active-modal" title="Active User" :show="false">
    <div class="p-4 rounded-lg relative">
        @include('user.partials.active-user')
    </div>
</x-modal>
